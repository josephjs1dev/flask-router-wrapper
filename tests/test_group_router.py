...
def create_blueprint_router(name, import_name, endpoint, http_methods, *handlers) -> BlueprintRouter:
...
def test_group_blueprint_router_in_router_success(app: Flask):
  http_methods = ["get", "post", "put", "patch", "delete"]
  blueprint_router = create_blueprint_router("blueprint", "blueprint", "", http_methods, index_handler) 
  router = Router()
